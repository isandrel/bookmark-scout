...
function PopupPage() {
  ...
  const toggleExpandAllChildren = useCallback(
    (node: BookmarkTreeNode, e: React.MouseEvent) => {
      e.stopPropagation();
  ...
  const filterFolders = useCallback(
    (nodes: BookmarkTreeNode[], query: string): BookmarkTreeNode[] => {
        ...
        if (node.children) {
          ...
          if (nodeMatches || filteredChildren.length > 0) {
            ...
            filtered.push({
              ...node,
              title: nodeMatches
                ? node.title.replace(new RegExp(`(${query})`, 'gi'), '<b>$1</b>')
                : node.title,
  ...
  const getParentFolderIds = useCallback(
    (nodes: BookmarkTreeNode[], targetId: string): string[] => {
      const result: string[] = [];
    ...
    async (operation: DragOperation) => {
      try {
        ...
        const targetFolder = operation.targetParentId
          ? await chrome.bookmarks.get(operation.targetParentId)
          : null;
  ...
  const renderFolderItem = (node: BookmarkTreeNode) => {
    ...
    const isAllChildrenExpanded =
      ...
      childFolderIds.every((id) => expandedFolders.includes(id));
                ...
                if (element) {
                  ...
                  const dropTargetCleanup = dropTargetForElements({
                    ...
                    onDrag: ({ source, location }) => {
                      ...
                      if (sourceData.node.id !== node.id) {
                        ...
                        const existingIndicator =
                          element.parentElement?.querySelector('.drop-indicator');
                    ...
                    onDrop: ({ source }) => {
                      ...
                      if (sourceData.node.id !== node.id) {
                        const closestEdge = element.dataset.closestEdge as
                          | 'top'
                          | 'bottom'
                          | undefined;
                        ...
                        if (sourceData.type === 'folder') {
                          ...
                          operationType = isDroppingIntoFolder
                            ? 'bookmark-move'
                            : 'bookmark-reorder';
                        }
                        ...
                        const operation: DragOperation = {
                          ...
                          targetIndex:
                            closestEdge === 'bottom' ? (node.index || 0) + 1 : node.index || 0,
                        };
            ...
            <div className="flex items-center gap-1 ml-2 shrink-0 action-button">
              {hasChildren && (
                <Button
                  ...
                  title={
                    isAllChildrenExpanded ? 'Collapse all subfolders' : 'Expand all subfolders'
                  }
                >
